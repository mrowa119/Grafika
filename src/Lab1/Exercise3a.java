...
public class Exercise3a extends MyImage implements Processable {
	...
	public void processImage(int i, int j) {
		...
		int distanceFromNearestCenter = getDistanceFromNearestCenter(distanceFromCenterOfVirtualSquareX, distanceFromCenterOfVirtualSquareY);
	...
	private int getDistanceFromNearestCenter(int distanceFromCenterOfVirtualSquareX,
		...
		return (int) Math.sqrt(Math.pow(distanceFromCenterOfVirtualSquareX, 2) + Math.pow(distanceFromCenterOfVirtualSquareY, 2));
	}
